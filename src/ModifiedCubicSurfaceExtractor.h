...
#ifndef __PolyVox_ModifiedCubicSurfaceExtractor_H__
#define __PolyVox_ModifiedCubicSurfaceExtractor_H__
...
namespace PolyVox
{
  ...
  //Added meshtype, modified default quadneeded
	...
	class ModifiedCubicSurfaceExtractor
	{
		struct IndexAndMaterial
		{
			int32_t iIndex;
      //Modified to meshtype
			MeshType uMaterial; //Should actually use the material type here, but this is ok for now.
		};
	...
	public:
    //Modified to add positionData
		ModifiedCubicSurfaceExtractor(VolumeType* volData, Region region, SurfaceMesh<PositionData<MeshType>>* result, bool bMergeQuads = true, IsQuadNeeded isQuadNeeded = IsQuadNeeded());


		void execute();		

	private:
		int32_t addVertex(float fX, float fY, float fZ, MeshType uMaterial, Array<3, IndexAndMaterial>& existingVertices);
    ...
    //Modification
		SurfaceMesh<PositionData<MeshType>>* m_meshCurrent;
...
//Changed path
#include "ModifiedCubicSurfaceExtractor.inl"

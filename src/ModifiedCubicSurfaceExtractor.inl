...
namespace PolyVox
{
	...
	const uint32_t ModifiedCubicSurfaceExtractor<VolumeType, MeshType, IsQuadNeeded>::MaxVerticesPerPosition = 6;

	template<typename VolumeType, typename MeshType, typename IsQuadNeeded>
	ModifiedCubicSurfaceExtractor<VolumeType, MeshType, IsQuadNeeded>::ModifiedCubicSurfaceExtractor(VolumeType* volData, Region region, SurfaceMesh<PositionData<MeshType>>* result, bool bMergeQuads, IsQuadNeeded isQuadNeeded)
		:m_volData(volData)
  ...
  //Added meshtype
	template<typename VolumeType, typename MeshType, typename IsQuadNeeded>
	void ModifiedCubicSurfaceExtractor<VolumeType, MeshType, IsQuadNeeded>::execute()
	{
		m_meshCurrent->clear();
		...
		for(int32_t z = m_regSizeInVoxels.getLowerCorner().getZ(); z <= m_regSizeInVoxels.getUpperCorner().getZ(); z++)
		{
          ...
          //Modified changed type
					MeshType material; //Filled in by callback
  ...
  //Added meshType
	template<typename VolumeType, typename MeshType, typename IsQuadNeeded>
	int32_t ModifiedCubicSurfaceExtractor<VolumeType, MeshType, IsQuadNeeded>::addVertex(float fX, float fY, float fZ, MeshType uMaterialIn, Array<3, IndexAndMaterial>& existingVertices)
	{
        ...
        //Changed to positionData
				rEntry.iIndex = m_meshCurrent->addVertex(PositionData<MeshType>(Vector3DFloat(fX, fY, fZ), uMaterialIn));
  ...
  //modified added MeshType
	template<typename VolumeType, typename MeshType, typename IsQuadNeeded>
	bool ModifiedCubicSurfaceExtractor<VolumeType, MeshType, IsQuadNeeded>::performQuadMerging(std::list<Quad>& quads)
	{
  ...
  //Modified MeshType
	template<typename VolumeType, typename MeshType, typename IsQuadNeeded>
	bool ModifiedCubicSurfaceExtractor<VolumeType, MeshType, IsQuadNeeded>::mergeQuads(Quad& q1, Quad& q2)
	{
		//All four vertices of a given quad have the same material,
		//so just check that the first pair of vertices match.
    
    //Modified throw an error
    assert(false);
